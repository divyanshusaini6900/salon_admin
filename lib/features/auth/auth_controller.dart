...
class AdminProfile {
  ...
  factory AdminProfile.fromMap(String uid, Map<String, dynamic> data) {
    return AdminProfile(
      uid: uid,
...
class AuthController extends StateNotifier<AuthState> {
  ...
  Future<void> _handleAuthChange(User? user) async {
    ...
    final doc = await store.collection('admin_users').doc(user.uid).get();
    if (doc.exists) {
      state = AuthState(isAuthenticated: true, profile: AdminProfile.fromMap(user.uid, doc.data()!));
    } else {
      state = AuthState(isAuthenticated: true, profile: AdminProfile(
        uid: user.uid,
        name: '',
        phone: '',
        dob: '',
  ...
  Future<void> signUp({
    ...
    try {
      ...
      final store = _firestore;
      if (store != null) {
        await store.collection('admin_users').doc(profile.uid).set(profile.toMap());
      }
      if (enableBiometric) {
